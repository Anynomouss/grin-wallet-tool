...
@author: Anynomous
Licence: Apache v2.0.
Dislaimer: Software is provided as it without any warranty. Use at your own risk
WARNINGs: 
...
def input_arguments_sanity_check(argumentss):
    ...
    if sum([argument.new, argument.load]) !=1:
        raise ValueError('A very specific bad thing happened.') ## Change all print statement to throw error <=
...
if __name__ == '__main__':
    ...
    ## are used, if not throw an error
    ###############################################################################

    ###########################################################################
    ## Wallet types
    parser.add_argument("--wallet-type", help = "Example: --wallet-tye=bip39-password-wallet", required = True, choices=["regular-wallet","bip39-password-wallet","bip39-wallet","voucher-wallet","brain-bip39-wallet"])
    ...
    ## Input and output, based on hether new or load is used, wallet-file and qr-file wil be read or created. Existing files will not be overwritten
    ...
    parser.add_argument("--wallet-file", help = "Example: --wallet-file='wallet_file_name", required = False, default = 'False', type=str)
    parser.add_argument("--qr-file", help = "Example: --qr-file='qr_file_name'", required = False, default = 'False', type=str)
